...
class DomainController extends Zend_Controller_Action
{
   ...
   public function indexAction() {
    ...
    public function editAction() {
        ...
        $allowlistelement = new Default_Model_WWElement();
        $allowlistform = $allowlistelement->fetchAll('@'.$view->domain->getParam('name'),'white');
	...
	$blocklistelement = new Default_Model_WWElement();
        $blocklistform = $blocklistelement->fetchAll('@'.$view->domain->getParam('name'),'black');
        $newslistelement = new Default_Model_WWElement();
        $newslistform = $newslistelement->fetchAll('@'.$view->domain->getParam('name'),'wnews');

    	$panelform = new $panelformclass($view->domain, $allowlistform, $warnlistform, $blocklistform, $newslistform, $this->getRequest());
        ...
        if ($this->getRequest()->isPost()) {
            if ($panelform->isValid($request->getPost())) {
		      ...
		      $view->domain->setParam('active', $request->get('enabledomain') == 1 ? "true" : "false");
                  ...
                  if ($panel == 'filtering') {
                      $panelform->_allowlist = $allowlistelement->fetchAll('@'.$view->domain->getParam('name'),'white');
                      $panelform->_warnlist = $warnlistelement->fetchAll('@'.$view->domain->getParam('name'),'warn');
                      $panelform->_blocklist = $blocklistelement->fetchAll('@'.$view->domain->getParam('name'),'black');
         ...
         $view->message = $message;
         $view->allowlistform = $allowlistform;
