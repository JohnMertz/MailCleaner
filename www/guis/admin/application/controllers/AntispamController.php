...
class AntispamController extends Zend_Controller_Action
{
	...
	$blocklistelement = new Default_Model_WWElement();
        $blocklist = $blocklistelement->fetchAll('','black');

    	$allowlistelement = new Default_Model_WWElement();
    	$allowlist = $whitelistelement->fetchAll('','white');
    	...
    	$form    = new Default_Form_AntispamGlobalSettings($antispam, $allowlist, $warnlist, $blocklist, $newslist);
        ...
        if ($request->isPost()) {
		  ...
		  $form->_blocklist = $blocklistelement->fetchAll('', 'black');
            	  $form->_allowlist = $allowlistelement->fetchAll('','white');
