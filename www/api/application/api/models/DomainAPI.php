...
class Api_Model_DomainAPI
{
	 ...
	 *     allowlists => can be 0 or 1, enable allowlists
	 *     warnlists => can be 0 or 1, enable warnlists
	 *     notice_wwlists => can be 0 or 1, enable administrator warning when allow or warn lists hit
	...
	private function setupParams($domain, $params) {
        ...
        foreach (array('allowlists' => 'enable_whitelists', 'warnlists' => 'enable_warnlists', 'notice_wwlists' => 'notice_wwlists_hit') as $key => $storekey) {
        	if (isset($params[$key]) && preg_match('/^[01]$/', $params[$key])) {
