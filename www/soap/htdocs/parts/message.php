...
/**
 * Add a spam to the allowlist as newsletter
 * @param $dest string original destination email address
...
/**
 * Add a spam to the allowlist
 * @param $dest string original destination email address
 * @param $sender string original sender
 * @return  string 'OK' or 'NOTOK'
 */
function addToAllowlist($dest, $sender) {
  if (!preg_match('/^\S+\@\S+$/', $dest)) {
      return "BADPARAMS";
  }
  if (!preg_match('/^\S*\@\S+$/', $sender)) {
  ...
  $dest = escapeshellarg($dest);
  $sender = escapeshellarg($sender);
  $cmd = $sysconf_->SRCDIR_."/bin/add_to_allowlist.pl $dest $sender";
  $res_a = array();
  exec($cmd, $res_a);
